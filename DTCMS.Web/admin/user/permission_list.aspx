...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        //操作权限
        var EditPermission=("<%=EditPermission %>"=="True");
        var DeletePermission=("<%=DeletePermission %>"=="True");
        var SettingPermission=("<%=SettingPermission %>"=="True");
    ...
    <form id="form1" runat="server">
        <div id="container">  
            <div id="tab_menu" class="tabs">
                <ul>
                    <li class="tab_on"><a href="javascript:;">角色管理</a></li>
                </ul>
            </div>
            <div class="toolbar">
        ...
        function deleteData(rid, flag) {//删除角色
            ...
            Dialog.confirm("删除角色将会影响到与之关联的用户不能正常使用后台功能，确定要删除吗？", function() {
                $.ajax({
                    url: "../ajax/permission_ajax.aspx",
                    type: "GET",
                    data: "action=delete&Id=" + rid + "&ran=" + Math.random(),
                    success: function(responseText) {//提示
                        if (responseText>0) {
                            showSuccess("成功删除"+responseText+"个角色！");
                            loadData(1);
                            return;
                        }
                        else {
