...
// 创建描述: DTCMS V1.0 创建于 2010-01-10 21:32:56
// 功能描述: 
// 修改标识: 
// 修改描述: 
//------------------------------------------------------------------------------
    ...
    public class UsersDAL : BaseDAL, IDAL_Users
    {
        ...
        public int Add(Users model)
        {
            ...
            SqlParameter[] cmdParms = {
				AddInParameter("@UserName", SqlDbType.VarChar, 50, model.UserName),
				AddInParameter("@NickName", SqlDbType.NVarChar, 50, model.NickName),
				AddInParameter("@Password", SqlDbType.VarChar, 64, model.Password),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT UID,UserName,RoleID,UsergroupID,IsVerify,IsLock FROM " + tablePrefix + "Users");
            strSql.Append(" WHERE UserName=@UserName AND Password=@Password");
            if (isAdmin)
            {
                //验证是否为系统用户组
                strSql.Append(" AND UsergroupID=1");
            }
            SqlParameter[] cmdParms ={
				AddInParameter("@UserName", SqlDbType.VarChar, 50, userName),
				AddInParameter("@Password", SqlDbType.VarChar, 64, pwd)};
