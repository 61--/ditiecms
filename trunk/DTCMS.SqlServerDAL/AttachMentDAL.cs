...
namespace DTCMS.SqlServerDAL
{
    public class AttachMentDAL:BaseDAL,IDAL_AttachMent
    {
        ...
        public int Add(AttachMent model)
        {
            ...
            strSql.Append("insert into DT_AttachMent(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update DT_AttachMent set ");
        ...
        {
            string strSql = string.Format("delete DT_AttachMent where AttachMentID in({0})",AttachMentID);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 AttachMentID,AttachMentAttribute,AttachMentDisplayName,AttachMentPath,AttachMentSize,AbbrPhotoPath,PubLisher,AddDate,PhotoDescription from DT_AttachMent ");
            strSql.Append(" where AttachMentID=@AttachMentID ");
            SqlParameter[] parameters = {
					new SqlParameter("@AttachMentID", SqlDbType.Int,4)};
            ...
            using (SqlDataReader dataRead = SqlHelper.ExecuteReader(CommandType.Text, strSql.ToString(), parameters))
            {
                if (dataRead.Read())
                {
                    model = DataReaderToModel<AttachMent>(dataRead);
                }
            }
            return model;
        }
