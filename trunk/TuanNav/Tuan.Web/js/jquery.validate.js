        ...
        */
        function validateInput(input, form) {
          ...
          assign_or_get_id = function(i) { if (!i.attr('id')) { i.attr('id', "input_" + parseInt(new Date().getTime() + Math.random(), 10)); } };
            ...
            while (i--) {
                if (input[0].attributes["validate"] != null) {
                    matches = input[0].attributes["validate"].nodeValue.match(compiled[i][0]);
                    if (matches) {
        ...
        return this.each(function() {
            var form = $(this);
            var inputs = $("input[type='checkbox'], input[type='password'], input[type='radio'].required, input[type='text'], select, textarea", form);
