...
Dialog.setBgDivSize = function() {
    ...
    if (Dialog.bgDiv) {
        if (isIE6) {
            Dialog.bgDiv.style.height = bd.height + "px";
            Dialog.bgDiv.style.top = bd.scrollTop + "px";
            Dialog.bgDiv.childNodes[0].style.display = "none"; //让div重渲染,修正IE6下尺寸bug
            Dialog.bgDiv.childNodes[0].style.display = "";
        } else {
            Dialog.bgDiv.style.width = bd.width + "px";
            Dialog.bgDiv.style.height = bd.height + "px";
        }
    }
};
...
Dialog.alert = function(msg, func, w, h) {
    ...
    diag.Title = "Tuanlet提示信息";
    diag.CancelEvent = function() {
        diag.close();
        if (func) func();
    };
    diag.InnerHtml = '<table height="100%" align="left" cellpadding="10" cellspacing="10" style="border-collapse:separate;">\
