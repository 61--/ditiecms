...
namespace DTCMS.SqlServerDAL
{
    ...
    public class Arc_ArticleDAL : BaseDAL, IDAL_Arc_Article
    {
        ...
        public DataTable GetDataTable(string fileds, string where)
        {
            ...
            if (where == "")
            {
                strSql = string.Format("SELECT {0} FROM {1}Arc_Article", fileds, tablePrefix);
            }
            else
            {
                strSql = string.Format("SELECT {0} FROM {1}Arc_Article WHERE {2}", fileds, tablePrefix, where);
            }
        ...
        /// <param name="count">返回记录数</param>
        /// <returns>对象泛型集合</returns>
        private List<Arc_Article> GetList(SqlDataReader dr, out int count)
        {
            count = 0;
        ...
        /// <param name="count">返回记录总数</param>
        /// <returns>分页对象泛型集合</returns>
        private List<Arc_Article> GetPageList(SqlDataReader dr, int pageSize, int pageIndex, out int count)
        {
