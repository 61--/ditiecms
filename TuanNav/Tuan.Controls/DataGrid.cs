...
namespace Tuan.Controls
{
    ...
    public class DataGrid : WebControl, INamingContainer
    {
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            output.WriteLine("function onSortClick(obj){if(obj.className=='asc'){sortValue=obj.id;isDesc=true;obj.className='desc';}else{sortValue=obj.id;isDesc=false;obj.className='asc';}if(s!=null&&s!=obj){s.className='nosort';}s=obj;loadData();}");
            output.Write("function showDataList(json){if(json.error<0){Dialog.alert(json.message);return false;}totalRecord=json.totalRecord;");
            if (this.IsPage)
            {
        ...
        private string BuildJavaScript()
        {
            ...
            js.Append("var totalRecord=-1;var s;var sortValue='';var isDesc=true;var keyWord='';\r\n");
            js.Append("$(document).ready(function(){loadData();});\r\n");
            js.Append("function loadData(sl){if(sl!=false){sl=true;};$.ajax({type:'GET',dataType:'json',url:'?r='+Math.random(),beforeSend:function(){if(sl){showLoading('正在加载数据，请稍候...','#dataList');}},complete:function(){hideMessage();},error:function(xhr,error){hideMessage();alert('错误信息:'+error);},success:function(data){showDataList(data);},data:{'op':'");
            js.Append(this.BindAjaxMethod);
            if (this.IsPage)
            {
                js.Append("','curPage':curPage,'pageSize':pageSize,'totalRecord':totalRecord,'sortValue':sortValue,'isDesc':isDesc,'keyWord':keyWord}});}\r\n");
                ...
                js.Append("if(curPage>totalPage){curPage=totalPage;return;}if(curPage<1){curPage=1;return;}loadData();}\r\n");
                js.Append("function setPageSize(opt){pageSize=opt[opt.selectedIndex].text;totalPage=Math.ceil(totalRecord/pageSize);if(curPage>totalPage)curPage=totalPage;loadData();}\r\n");
                js.Append("function jumpPage(val,e){e=e||event;val=parseInt(val);if(e.keyCode==13&&val>0&&val<=totalPage&&val!=curPage){curPage=val;loadData();}}\r\n");
                js.Append("document.onkeydown=function(e){e=e||event;if(e.keyCode==37){curPage--;}else if(e.keyCode==39){curPage++;}else{return;}if(curPage>totalPage){curPage=totalPage;return;}if(curPage<1){curPage=1;return;}loadData();}\r\n");
            }
            else
            {
                js.Append("','sortValue':sortValue,'isDesc':isDesc,'keyWord':keyWord}});}\r\n");
            }
